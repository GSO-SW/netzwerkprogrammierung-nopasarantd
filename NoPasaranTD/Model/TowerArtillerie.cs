...
namespace NoPasaranTD.Model
{
    public class TowerArtillerie : Tower
    {
        ...
        SolidBrush bruhBlack, bruhRed, bruhPurple, bruhLightGray, bruhFireColor, bruhDarkGray, bruhSlateGray, bruhWhite;
        Pen penBlack, penRed, penPurple, penWhite, penSlateGray;
        Font font;
        ...
        //animation related
        int phaseUp = 1; // 1:reloading | 2:firing | 3:waiting      | --> dominant
        ...
        public TowerArtillerie()
        {
            ...
            penWhite = new Pen(Color.White); penSlateGray = new Pen(Color.SlateGray);
        ...
        public override void Render(Graphics g)
        {
            ...
            float brickSizeX = rectPipe.X * 0.02f;
            ...
            switch (phaseUp)
            {
                ...
                case 2: // animation: fire!!
                    if (phaseDown != 1 && phaseDown != 2) // [!phaseDown2] could be optional
                    {
                        ...
                        float TS3 = TweenServiceExp(tickFireStart,tickFireStart+tickLength_fireAnimation,20,800,lTicks,10);
                        RectangleF rect = new RectangleF(
                            centerCanon.X - TS3, centerCanon.Y - TS3,
                            TS3*2, TS3*2
                            );
                        g.FillEllipse(bruhSlateGray, rect);
        ...
        private static float TweenServiceExp(ulong tickStart, ulong tickEnd, float minValue, float maxValue, ulong currentTick, float power)
        {
            float deltaValue = maxValue - minValue, deltaTick = tickEnd - tickStart;
            ulong d = currentTick < tickStart ? 0 : currentTick - tickStart;
            float factor = Math.Min(d, deltaTick) / deltaTick;  //  E[0;1]
            //Console.WriteLine(factor);
            return minValue + deltaValue * (float)Math.Pow(factor, power);
        }
